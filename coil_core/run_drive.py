...
from utils.general import compute_average_std_separatetasks, get_latest_path, \
    write_header_control_summary, \
...
def start_carla_simulator(gpu, town_name, no_screen, docker):
    ...
    carla_out_file_err = os.path.join('_output_logs',
                                      'CARLA_err_' + g_conf.PROCESS_NAME + '_' + str(
                                          os.getpid()) + ".out")
    ...
    if docker:
        ...
        sp = subprocess.Popen(['docker', 'run', '--rm', '-d', '-p',
                               str(port) + '-' + str(port + 2) + ':' + str(port) + '-' + str(
                                   port + 2),
                               '--runtime=nvidia', '-e', 'NVIDIA_VISIBLE_DEVICES=' + str(gpu),
                               'carlagear',
        ...
        if not no_screen:
            sp = subprocess.Popen(
                [carla_path + '/CarlaUE4/Binaries/Linux/CarlaUE4', '/Game/Maps/' + town_name,
                 '-windowed',
                 '-benchmark', '-fps=10', '-world-port=' + str(port)], shell=False,
    ...
    coil_logger.add_message('Loading', {'CARLA': '/CarlaUE4/Binaries/Linux/CarlaUE4'
                                                 '-windowed' + '-benchmark' + '-fps=10' + '-world-port=' + str(
        port)})
    ...
    try:
        ...
        if params['suppress_output']:
            sys.stdout = open(os.path.join('_output_logs',
                                           exp_alias + '_' + drive_conditions + '_' + str(
                                               os.getpid()) + ".out"),
                              "a", buffering=1)
            sys.stderr = open(os.path.join('_output_logs',
                                           exp_alias + '_err_' + drive_conditions + '_' + str(
                                               os.getpid()) + ".out"),
                              "a", buffering=1)
            ...
            try:
                ...
                if is_next_checkpoint_ready(g_conf.TEST_SCHEDULE,
                    ...
                    path = exp_batch + '_' + exp_alias + '_' + str(latest) \
                           ...
                           + '_' + g_conf.PROCESS_NAME.split('_')[1] + '_' + \
                           g_conf.PROCESS_NAME.split('_')[2]
                    ...
                    averaged_dict, std_dict = compute_average_std_separatetasks([benchmark_dict],
                                                                                ...
                                                                                len(
                                                                                    experiment_set.build_experiments()),
                                                                                number_of_reps=
                                                                                experiment_set.build_experiments()[
                                                                                    0].repetitions)
