    ...
    def __call__(self, images):
        ...
        if per_channel == 1:
            ...
            for c, sample in enumerate(samples):
                ...
                image = F.multiply(image[c, ...].cuda(), torch.ones(1).cuda() * float(sample))
        else:
            sample = self.mul.draw_samples((nb_images,), random_state=rs_image).astype(np.float32)
            ch.do_assert(sample.all() >= 0)

            print (torch.squeeze(images).shape)
            print (sample.shape)

            result = F.multiply(torch.squeeze(images.cuda()), torch.from_numpy(sample))
