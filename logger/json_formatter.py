...
This package was created to leverage the very flexible standard Python logging package to produce
JSON logs. JSON allows for combining multiple types of records in one stream which can easily be
filtered or viewed together as needed.
...
def filelogger(logname, recordfields=[], filename='json.log', level=INFO, writing_level='w'):
    ...
    Args:
         logname      :   The name of the logger - to allow for multiple logs, and levels of logs in
                          an application
...
def streamlogger(logname, recordfields=[], outputstream=None, level=INFO):
    """A convenience function to return a JSON stream logger for simple situations.
        ...
        Args:
         logname      :   The name of the logger - to allow for multiple logs, and levels of logs
                           in an application
         recordfields :   The metadata fields to add to the JSON record created by the logger
         outputstream :   The outputstream to be used by the logger. sys.stderr is used when
                           outputstream is None.
    Returns:
...
def readJSONlog(logfile, filterfunction=(lambda x: True), customjson=None):
    """Iterate through a log file of JSON records and return a list of JSON records that meet the
    filterfunction.

    Args:
        logfile          : A file like object consisting of JSON records.
        filterfunction   : A function that returns True if the JSON record should be included in the
    ...
    json_records = []
    for x in logfile:
        # if the record in the logfile returns true from the filter function
        # convert it to JSON and add it the records to return
...
class JSONFormatter(Formatter):
    """The JSONFormatter class outputs Python log records in JSON format.
       ...
       JSONFormatter assumes that log record metadata fields are specified at the fomatter level as
       opposed to the record level. The specification of matadata fields at the formatter level
       allows for multiple handles to display differing levels of detail. For example, console log
       output might specify less detail to allow for quick problem
       triage while file log output generated from the same data may contain more detail for
       in-depth investigations.

       Attributes:
           recordfields  : A list of strings containing the names of metadata fields (see Python
                           log record documentation for details) to add to the JSON output. Metadata
                            fields will be added to the JSON record in
    ...
    def __init__(self, recordfields=[], datefmt=None, customjson=None):
        """__init__ overrides the default constructor to accept a formatter specific list of
        metadata fields

        Args:
            recordfields : A list of strings referring to metadata fields on the record object. It
                           can be empty. The list of fields will be added to the JSON record created
                            by the formatter.
        """
    ...
    def usesTime(self):
        """ Overridden from the ancestor to look for the asctime attribute in the recordfields
        attribute.

        The override is needed because of the change in design assumptions from the documentation
        for the logging module. The implementation in this object could be brittle if a new release
        changes the name or adds another time attribute.
    ...
    def _getjsondata(self, record):
        """ combines any supplied recordfields with the log record msg field into an object to
            convert to JSON

            Args:
                record   : log record to output to JSON log
            Returns:
                An object to convert to JSON - either an ordered dict if recordfields are supplied
                or the record.msg attribute
        """
        if len(self.recordfields) > 0:
            ...
            # An OrderedDict is used to ensure that the converted data appears in the
    ...
    def format(self, record):
        """overridden from the ancestor class to take a log record and output a JSON formatted
            string.
