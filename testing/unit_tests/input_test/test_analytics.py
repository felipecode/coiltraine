...
class testSpliter(unittest.TestCase):
    ...
    def test_split_real_data_speed(self):
        ...
        mask = analytics.get_data_conditioned(
            conditions_vec, dataset.measurements, dataset.meta_data)

        print (" The sum is ", np.sum(dataset.measurements[np.where(
            dataset.meta_data[:, 0] == b'speed_module'), mask]))
