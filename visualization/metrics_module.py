...
def aggregate_metric(metric_raw, param):
    if np.isscalar(metric_raw):
        ...
        if param['type'] == 'mean':
            ...
            metric_result = float(np.sum(metric_mask)) / float(len(metric_raw_np))
        else:
...
def compute_km_per_infraction(data_item, param):
    label_to_index = {'intersection_offroad': 1, 'intersection_otherlane': 2,
                      'collision_pedestrians': 3, 'collision_vehicles': 4}
...
def compute_displacement_steer(data_item, param):
    ...
    steer_pred = data_item['steer_pred'][np.where(
        abs(data_item['steer_gt']) > (param['thresh_steer']))]
...
def compute_count_errors_weighted(data_item, param):
    ...
    count = float(sum(abs(data_item['steer_gt'] - data_item['steer_pred'])
                      > weighted_gt)) / float(len(weighted_gt))
